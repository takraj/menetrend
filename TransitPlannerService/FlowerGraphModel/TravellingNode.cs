...
namespace FlowerGraphModel
{
    ...
    {
        protected int _stopId, _sequenceId;
        protected DateTime _baseTime;

        public TravellingNode(FlowerGraph graph, DijkstraPathfinderState state, int stopId, int sequenceId, DateTime baseTime) : base(graph, state)
        {
            _stopId = stopId;
        ...
        public override IEnumerable<KeyValuePair<FlowerNode, DateTime>> GetNeighbours()
        {
            ...
            {
                var nextNode = new TravellingNode(_graph, _state, lookupResult.NextStopIdx, _sequenceId, _baseTime);
