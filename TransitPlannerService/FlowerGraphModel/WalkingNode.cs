...
namespace FlowerGraphModel
{
    public class WalkingNode : FlowerNode
    {
        ...
        public WalkingNode(FlowerGraph graph, DijkstraPathfinderState state, int stopId) : base(graph, state)
        {
            _stopId = stopId;
        }
                            ...
                            if (now <= endtime)
                            {
                                ...
                                if ((serviceDay >= 0) && (_graph.Repository.IsServiceAvailableOnDay(trip.ServiceIdx, serviceDay)))
                                {
                                    ...
                                    if (tripBaseTime <= maxDeparture)
                                    {
                                        ...
                                        {
                                            var node = new TravellingNode(_graph, _state, _stopId, sequenceId, tripBaseTime);
